...
When no single field qualifies, a _composite_ candidate key (the minimum set of fields that together are unique) is valid — but keep it to the minimum; extra fields disqualify it. A candidate key that qualifies but is not chosen as the primary key becomes an _alternate key_, and its uniqueness should still be enforced.
