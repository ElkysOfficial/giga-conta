...
Set the foreign key's uniqueness to match the relationship: _non-unique_ for one-to-many (one parent value relates to many child records), _unique_ for one-to-one. Unlike the primary key, foreign-key values are entered by the user, so its range of values is constrained to the existing primary-key values.
