...
A relational database is never completely free of redundant data (foreign keys repeat by design); the goal is the _minimum_. Elements 3 and 4 are usually resolved when refining fields, but re-check them at the table level — a multivalued or calculated field is easy to miss on the first pass. Load a few rows of realistic sample data to confirm: anomalies surface immediately when you try to sort, filter, or update.
