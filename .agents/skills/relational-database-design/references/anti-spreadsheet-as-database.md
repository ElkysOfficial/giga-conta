...
The wrong default is to carry a spreadsheet straight into a database — one sheet becomes one table, its columns become fields, its visual layout becomes the structure. A spreadsheet is built to store data exactly as it is displayed; a relational database separates how data is _stored_ from how it is _presented_. Modeling the storage on the display reproduces every spreadsheet defect: duplicate fields, multipart cells, multivalued cells, and no key.
