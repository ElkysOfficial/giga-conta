...
This rule is about _ordering_: do the logical design first, choose and target an RDBMS second. The wrong default collapses the two — designing tables and rules against the product already in hand, its data types and constraint support standing in for the requirements. An RDBMS gives you tools to _implement_ a design; it gives you no principles or rationale for _creating_ one, so a product-first order lets the tool's shape substitute for the organization's information requirements.
...
(The failure _symptoms_ of the reverse order — decisions bent to what the product does, a schema bounded by your skill with it — are catalogued in [`anti-rdbms-driven-design`](anti-rdbms-driven-design.md); this rule is the ordering principle that avoids them.)
